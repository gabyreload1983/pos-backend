...
export async function abrirCaja(connection, data) {
  const [result] = await connection.query(
    "INSERT INTO cajas (usuario_id, fecha_apertura, saldo_inicial) VALUES (?, ?, ?)",
...
export async function cerrarCaja(connection, caja_id, data) {
  await connection.query(
    "UPDATE cajas SET fecha_cierre = ?, saldo_final = ? WHERE id = ?",
...
export async function obtenerCajaAbierta(sucursal_id) {
  const [rows] = await pool.query(
    "SELECT * FROM cajas WHERE sucursal_id = ? AND fecha_cierre IS NULL ORDER BY fecha_apertura DESC LIMIT 1",
    [sucursal_id]
  );
  return rows[0];
}
