...
export async function obtenerTotalSistema(caja_id) {
  const [ingresos] = await pool.query(
    "SELECT SUM(monto) AS total FROM movimientos_caja WHERE caja_id = ? AND tipo = 'ingreso'",
    [caja_id]
  );
  const [egresos] = await pool.query(
    "SELECT SUM(monto) AS total FROM movimientos_caja WHERE caja_id = ? AND tipo = 'egreso'",
    [caja_id]
  );
